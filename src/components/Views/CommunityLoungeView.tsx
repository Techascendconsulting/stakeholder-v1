      ...
      <div className="w-64 bg-gray-50 dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          ...
          {currentSection === 'threads' && (
            <div className="p-4">
              <div className="mb-6">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-1">Threads</h2>
                <p className="text-sm text-gray-600 dark:text-gray-400">Continue conversations and discussions</p>
              </div>

              <div className="space-y-3">
                {messages.filter(msg => messageReplyCounts[msg.id] > 0).map((message) => {
                  // Get the latest reply to show preview
                  const latestReply = messages
                    .filter(msg => msg.replied_to_id === message.id)
                    ...
                    >
                      <div className="flex items-start space-x-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {latestReply && (
                            <div className="flex items-start space-x-2 p-2 bg-gray-50 dark:bg-gray-700/50 rounded border-l-2 border-blue-200 dark:border-blue-700">
                              <div className="w-5 h-5 bg-gradient-to-br from-green-500 to-emerald-600 rounded-full flex items-center justify-center text-white text-xs font-bold">
                                {latestReply.user?.display_name?.charAt(0) || latestReply.user?.email?.charAt(0) || 'U'}
                              </div>
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center space-x-2 mb-1">
                                  <span className="text-xs font-medium text-gray-700 dark:text-gray-300">
                                    {latestReply.user?.display_name || latestReply.user?.email?.split('@')[0] || 'User'}
                                  </span>
                                  <span className="text-xs text-gray-500 dark:text-gray-400">
                ...
                {messages.filter(msg => messageReplyCounts[msg.id] > 0).length === 0 && (
                  <div className="p-8 text-center bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
                    ...
                    <h3 className="text-sm font-medium text-gray-900 dark:text-white mb-1">No threads yet</h3>
                    <p className="text-xs text-gray-500 dark:text-gray-400 mb-4">Start a conversation by replying to a message</p>
                    <button 
                      onClick={() => setCurrentSection('channels')}
