...
import { stakeholderInterviewBlueprint, StakeholderInterviewState } from '../data/stakeholderInterviewBlueprint';
import { StakeholderInterviewSession, generateSummary } from '../services/stakeholderInterviewReducer';
...
const StakeholderInterviewPanel: React.FC<StakeholderInterviewPanelProps> = ({
  session,
  onNext,
  onSendSummary,
  onAddPainPoint,
  ...
  console.log('🎯 DEBUG - StakeholderInterviewPanel render:', {
    sessionState: session.state,
    sessionProgress: session.progress,
    sessionData: session.data
  });
  const [activeTab, setActiveTab] = useState<'guide' | 'dig-deeper' | 'interpret' | 'notes' | 'artifacts'>('guide');
  const [summaryText, setSummaryText] = useState('');

  const currentState = stakeholderInterviewBlueprint.states[session.state];
  ...
  const tabs = [
    ...
    { id: 'artifacts', label: 'Artifacts', icon: CheckSquare }
  ] as const;
  ...
  const renderGuideTab = () => (
    <div className="space-y-4">
      ...
      {(() => {
        const isEarlyPhase = session.state === 'warm_up' || session.state === 'problem_exploration';
        const hasContent = session.data.pain_points.length > 0 || 
                          (session.data.priority_choice && session.data.priority_choice.trim() !== '') || 
                          session.data.success_measures.length > 0;
        const shouldShow = !isEarlyPhase && hasContent;
        
        console.log('🔍 DEBUG - Summarize visibility check:', {
          currentState: session.state,
          isEarlyPhase,
          hasContent,
          shouldShow,
          painPoints: session.data.pain_points.length,
          priorityChoice: session.data.priority_choice,
          priorityChoiceLength: session.data.priority_choice?.length,
          successMeasures: session.data.success_measures.length,
          sessionData: session.data
        });
        
        // Force hide during warm_up for debugging
        if (session.state === 'warm_up') {
          console.log('🚫 DEBUG - Forcing hide during warm_up phase');
          return false;
        }
        ...
        <div className="space-y-3">
          {currentState.cards.dig_deeper.map((question, index) => (
            <div key={index} className="bg-gray-50 dark:bg-gray-700 rounded-lg p-3">
              ...
              {question.when && question.when.length > 0 && (
  ...
  const renderInterpretTab = () => (
    <div className="space-y-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
        ...
        <div className="space-y-4">
          <div>
            <h4 className="font-medium text-green-700 dark:text-green-300 text-sm mb-2 flex items-center">
          ...
          <div>
            <h4 className="font-medium text-red-700 dark:text-red-300 text-sm mb-2 flex items-center">
          ...
          <div>
            <h4 className="font-medium text-blue-700 dark:text-blue-300 text-sm mb-2 flex items-center">
  ...
  const renderArtifactsTab = () => (
    <div className="space-y-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-3">Interview Progress</h3>
        
        <div className="space-y-3">
          {Object.entries(stakeholderInterviewBlueprint.states).map(([stateId, state]) => (
      ...
      {session.data.priority_choice && (
        <div className="bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
          ...
          <div className="bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-700 rounded-lg p-3">
            <p className="text-sm font-medium text-purple-900 dark:text-purple-100">{session.data.priority_choice}</p>
          </div>
        </div>
      )}
      ...
      {session.data.success_measures.length > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-3">Success Measures</h3>
          <div className="space-y-2">
            {session.data.success_measures.map((measure, index) => (
              <div key={index} className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-700 rounded-lg p-2">
                <p className="text-sm text-green-900 dark:text-green-100">{measure}</p>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="bg-gray-50 dark:bg-gray-900 border-l border-gray-200 dark:border-gray-700 w-96 flex flex-col">
      ...
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 p-4">
        <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Stakeholder Interview</h2>
...
export default StakeholderInterviewPanel;
