...
class AIService {
  ...
  public async generateInterviewNotes(meetingData: any, progressCallback?: (progress: string) => void): Promise<string> {
    try {
      ...
      const systemPrompt = "You are a professional Business Analyst creating meeting notes from a stakeholder interview. Create a factual, concise summary based ONLY on what was actually discussed. Include meeting context (date, duration, participants, project), then summarize the specific topics and points that were actually mentioned. Do NOT add information that was not discussed. Keep it brief and factual. If the conversation was short, acknowledge this and focus only on what was said. Use plain text formatting - NO asterisks, markdown, or special formatting.";

      const conversationText = messages.map((msg: any) => msg.speaker + ": " + msg.content).join("\n\n");
      const userPrompt = "Project: " + (project?.name || "Unknown Project") + " Duration: " + (duration || 0) + " minutes Participants: " + (participants?.map((p: any) => p.name + " (" + p.role + ")").join(", ") || "Unknown") + " Conversation: " + conversationText + " Please create a factual summary based ONLY on what was actually discussed. Be concise and accurate. Do NOT add information that was not mentioned in the conversation. If the discussion was brief, keep the summary brief. Use plain text formatting - NO asterisks, markdown, or special formatting.";
