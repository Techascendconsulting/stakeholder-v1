#!/bin/bash
# Add generateInterviewNotes method to aiService.ts

# Find the line before the closing brace
line_num=$(grep -n "^}$" src/services/aiService.ts | tail -1 | cut -d: -f1)

# Create the method content
method_content="  public async generateInterviewNotes(
    meetingData: any,
    ...
    try {
      ...
      // Check if there's enough meaningful content
