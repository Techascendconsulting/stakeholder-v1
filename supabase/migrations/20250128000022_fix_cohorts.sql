...
-- CREATE POLICY "cohorts_insert_admin" ON public.cohorts FOR INSERT WITH CHECK (auth.uid() IN (SELECT id FROM public.profiles WHERE role = 'admin'));
-- CREATE POLICY "cohorts_update_admin" ON public.cohorts FOR UPDATE USING (auth.uid() IN (SELECT id FROM public.profiles WHERE role = 'admin'));
-- CREATE POLICY "cohorts_delete_admin" ON public.cohorts FOR DELETE USING (auth.uid() IN (SELECT id FROM public.profiles WHERE role = 'admin'));
