...
async function testStakeholderAI() {
  try {
    const response = await makeRequest('/api/stakeholder-reply', {
      method: 'POST',
      body: {
        storyId: 'test',
        chat: [
          { role: 'user', content: 'What are your main concerns?' }
        ],
        scenarioContext: 'General business meeting'
      }
    });
    ...
    const hasResponse = response.body && response.body.content && response.body.content.trim().length > 0;
    recordResult('Stakeholder Meeting', hasResponse,
      hasResponse ? `Response received (${response.body.content.substring(0, 30)}...)` : 'No response');
    return hasResponse;
  } catch (error) {
