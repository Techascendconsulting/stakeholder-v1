...
const DownloadButton: React.FC = () => {
  const downloadAsPNG = async () => {
    try {
      ...
      // Capture the entire page with full dimensions
      ...
      link.download = 'business-analyst-fundamentals.png';
...
<head>
    ...
    <title>Business Analyst Fundamentals</title>
    ...
    <style>
        ...
        .btn-hover:hover { transform: translateX(4px); }
    </style>
</head>
<body class="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50">
    ...
    a.href = url;
    a.download = 'business-analyst-fundamentals.html';
  ...
  return (
    <div id="download-section" className="flex flex-col sm:flex-row gap-4 items-center">
      <button
        onClick={downloadAsPNG}
        className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-300"
      >
      ...
      <div className="max-w-7xl mx-auto px-6 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
          {concepts.map((concept) => (
            <div
              key={concept.id}
              className={`group relative bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100 cursor-pointer card-hover ${
