-- ====================================================================
-- UNIFIED COMMUNITY HUB MIGRATION
-- Integrates existing forum structure with new community features
-- Run this in your Supabase SQL Editor
-- ====================================================================

-- 1. Check existing community tables
SELECT 'Checking existing community tables:' as info;
SELECT table_name, table_schema 
FROM information_schema.tables 
WHERE table_schema = 'public' 
AND table_name IN ('forum_posts', 'forum_replies', 'forum_reactions', 'forum_post_tags', 'forum_reports', 'forum_user_stats', 'profiles', 'groups', 'group_members', 'buddy_pairs', 'training_sessions')
ORDER BY table_name;

-- 2. Create profiles table (if not exists) - enhanced version for community
...
-- 5. Create Groups table for community management
...
-- 7. Create Buddy Pairs table for student pairing
...
-- 15. Migrate existing users to profiles table (basic version first)
INSERT INTO public.profiles (id, email, full_name, role, is_admin, is_super_admin, is_senior_admin, locked, registered_device)
SELECT 
  au.id,
  au.email,
...
ON CONFLICT (id) DO UPDATE SET
  ...
  is_senior_admin = EXCLUDED.is_senior_admin,
  locked = EXCLUDED.locked,
  registered_device = EXCLUDED.registered_device;

-- 16. Create sample community data
...
AND NOT EXISTS (
  ...
  WHERE (bp.user1_id = p1.id AND bp.user2_id = p2.id) 
  OR (bp.user1_id = p2.id AND bp.user2_id = p1.id)
)
LIMIT 1;

-- 20. Create sample forum posts for community engagement
INSERT INTO public.forum_posts (user_id, content, category, system_generated)
SELECT 
  (SELECT id FROM public.profiles WHERE role = 'admin' LIMIT 1),
  'Welcome to the BA Community! 🎉 This is your space to connect, learn, and grow together. Feel free to ask questions, share experiences, and support each other on your BA journey.',
  'general',
  true
WHERE NOT EXISTS (SELECT 1 FROM public.forum_posts WHERE system_generated = true AND category = 'general');

-- 21. Show the results
SELECT 'Unified Community Hub Migration Complete!' as status;
SELECT 'Tables created/updated:' as info;
SELECT table_name FROM information_schema.tables 
WHERE table_schema = 'public' 
AND table_name IN ('profiles', 'groups', 'group_members', 'buddy_pairs', 'training_sessions', 'forum_posts', 'forum_replies', 'forum_reactions', 'forum_post_tags', 'forum_reports', 'forum_user_stats')
ORDER BY table_name;
...
SELECT 
  ...
  (SELECT COUNT(*) FROM public.buddy_pairs) as buddy_pairs_count,
  (SELECT COUNT(*) FROM public.forum_posts) as forum_posts_count;

-- 22. Show user roles distribution
