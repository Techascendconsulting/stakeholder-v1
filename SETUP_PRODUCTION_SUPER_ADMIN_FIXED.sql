-- Complete setup for production Super Admin (FIXED VERSION)
...
-- Step 6: Set up your business email as Super Admin (FIXED)
...
BEGIN
  ...
  IF user_exists THEN
    -- Get the user ID (using different variable name to avoid conflict)
    SELECT id INTO target_user_id FROM auth.users WHERE email = 'techascendconsulting@gmail.com';
    
    -- Update or insert into user_profiles (using target_user_id to avoid ambiguity)
    INSERT INTO user_profiles (user_id, is_super_admin, is_admin, is_senior_admin, blocked, locked)
    VALUES (target_user_id, TRUE, TRUE, TRUE, FALSE, FALSE)
    ON CONFLICT (user_id) 
    DO UPDATE SET 
      is_super_admin = TRUE,
      is_admin = TRUE, 
