...
import { Users, MessageSquare, FileText, Clock, Award, Target, Calendar, ChevronRight, Play, FolderOpen, ArrowRight, Lightbulb, Zap, BookOpen, RefreshCw, CheckCircle, AlertCircle, Rocket, Map, Brain } from 'lucide-react';
...
const Dashboard: React.FC = () => {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      console.log('🎯 Dashboard - Loading journey progress...');
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${
                ...
                {nextStep.icon === 'career' && <Map className="w-6 h-6 text-white" />}
                {nextStep.icon === 'learning' && <Brain className="w-6 h-6 text-white" />}
                {nextStep.icon === 'practice' && <Target className="w-6 h-6 text-white" />}
                {nextStep.icon === 'project' && <Rocket className="w-6 h-6 text-white" />}
              </div>
              <div>
